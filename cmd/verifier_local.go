...
// runLocalVerifierWizard adds a verifier to hud.yaml interactively, asking
// for each field one at a time. Pre-set values from the parent command's
// --name/--direction/--type/--permissions flags become the defaults the
// wizard suggests, so power users can pre-fill the bits they already know.
//
// Stdin and stdout come from the cobra command so the flow is testable
// with a scripted reader.
