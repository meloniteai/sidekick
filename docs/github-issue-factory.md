...
The action validates the isolated `$HOME/.codex/hooks.json` generated by
`sidekick install --codex --no-claude` and refuses to continue unless every hook
command is exactly `sidekick hook write`. It then asks Codex `hooks/list` for
that hook's current hash and writes the matching `trusted_hash` into the
isolated `$HOME/.codex/config.toml`, so headless Codex runs a genuinely trusted
hook rather than relying on `--dangerously-bypass-hook-trust`.
