...
outside the Codex command sandbox, so backend telemetry and write-hook IPC keep
working while model-generated shell commands stay sandboxed.
...
with `--container-options "--privileged"` by default so Codex's Linux sandbox
can create its nested namespace inside Docker. Set
`ACT_LIVE_CONTAINER_OPTIONS=0` to opt out, or set it to another Docker options
string for local experimentation. This does not change the Codex sandbox mode
passed to the workflow; live runs still use `workspace-write`.
